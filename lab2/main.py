...
def main_menu(msg='') -> int:
    menu = consolemenu.SelectionMenu(['Зареєструватися', 'Увійти'],
                                     title=f'ГОЛОВНЕ МЕНЮ {f"({msg})" if msg else ""}')
    menu.show()
...
def user_menu(login) -> int:
    menu = consolemenu.SelectionMenu(
        ['Вийти', 'Надіслати повідомлення', 'Вхідні повідомлення', 'Статистика моїх повідомлень'],
        title=f'КОРИСТВАЦЬКЕ МЕНЮ ДЛЯ <{login}>', subtitle='Оберіть опцію меню')
    menu.show()
    if menu.is_selected_item_exit():
        exit()
    return menu.selected_option + 1
...
def main():
    ...
    while loop:
        ...
        if choice == 1:
            if not signed_in:
                login = input('Введіть свій нікнейм: ')
            ...
            if signed_in:
                ...
                recipient = input('Введіть нікнейм отримувача: ')

                if create_message(connection, message, current_user_id, recipient):
                    print('Надсилання повідомлення...')
            else:
                login = input('Введіть свій нікнейм: ')
                ...
                if signed_in:
                    ...
                    menu = partial(main_menu, f'Користувача <{login}> ще не зареєстрований')
            ...
            print('В чіерзі: %s\nНа перевірці: %s\nЗаблоковано: %s\nНадіслано: %s\nДоставлено: %s' %
                  tuple(current_user))
            input('Натисніть ЕНТРЕТ щоб продовжити...')
