...
class User(Thread):
    ...
    def run(self):
        while True:
            ...
            print(f'користувач <{self.username}`> надіслав повідомлення до <{receiver}>\n<{message_text}>\n')
            time.sleep(1)
...
if __name__ == '__main__':
    ...
    fake = Faker()
    users_count = int(input('Введіть кількість користувачів для емуляції: '))
    ...
    input('Натисність ЕНТЕР для початку...')
    for t in threads:
        t.start()
